...
JSDragDropTree.prototype = {
    ...
    */
    ...
    deleteItem: function (obj1, obj2) {
        var message = 'Esta seguro de eliminar el nodo ' + obj2.innerHTML;
        if (this.hasSubNodes(obj2.parentNode)) message = message + ' y todas sus dependencias';
        if (confirm(message)) {
    ...
    __deleteItem_step2: function (obj) {
        ...
        JSTreeObj.ajaxObjects[ajaxIndex].requestFile = JSTreeObj.filePathDeleteItem + saveString; // Specifying which file to get
    ...
    __deleteComplete: function (ajaxIndex, obj) {
        if (this.ajaxObjects[ajaxIndex].response != 'OK') {
            alert('Error al tratar de eliminar el nodo: ' + this.ajaxObjects[ajaxIndex].response); 	// Rename failed
        } else {
    ...
    __renameComplete: function (ajaxIndex) {
        if (this.ajaxObjects[ajaxIndex].response != 'OK') {
            alert('Error al tratar de renombrar el nodo: ' + this.ajaxObjects[ajaxIndex].response); 	// Rename failed
        }
    }
		,
    ...
    initTree: function () {
        ...
        if (this.deleteAllowed || this.renameAllowed || this.addAllowed) {
            try {
                ...
                if (this.deleteAllowed) menuModel.addItem(1, 'A&ntildeadir', '', '', false, 'JSTreeObj.addItem');
                if (this.deleteAllowed) menuModel.addItem(2, 'Eliminar', '', '', false, 'JSTreeObj.deleteItem');
                if (this.renameAllowed) menuModel.addItem(3, 'Renombrar', '', '', false, 'JSTreeObj.renameItem');
                menuModel.init();

                var menuModelRenameOnly = new DHTMLGoodies_menuModel();
                if (this.renameAllowed) menuModelRenameOnly.addItem(4, 'Renombrar', '', '', false, 'JSTreeObj.renameItem');
                menuModelRenameOnly.init();

                var menuModelDeleteOnly = new DHTMLGoodies_menuModel();
                if (this.deleteAllowed) menuModelDeleteOnly.addItem(5, 'Eliminar', '', '', false, 'JSTreeObj.deleteItem');
                menuModelDeleteOnly.init();

                var menuModelAddOnly = new DHTMLGoodies_menuModel();
                if (this.addAllowed) menuModelAddOnly.addItem(6, 'A&ntildeadir', '', '', false, 'JSTreeObj.addItem');
        ...
        for (var no = 0; no < menuItems.length; no++) {
            ...
            if (this.contextMenu) {
                ...
                if (noRename == 'true' && noDelete == 'true' && noAdd == 'true') { } else {
                    ...
                    else if (noAdd == 'true') this.contextMenu.attachToElement(aTag, false, menuModelAddOnly);
