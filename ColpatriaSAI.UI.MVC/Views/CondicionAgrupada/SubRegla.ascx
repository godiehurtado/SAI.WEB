 ...
 <table id="tablaLista5">
        ...
        <tbody>
            ...
            <% foreach (var item in ((IEnumerable<SubRegla>)ViewData["SubReglas"]))
               { %>
            <tr>
                <td align="center"><%: Html.RadioButton("seleccionSubRegla", item.id, new { @onclick = "seleccionarSubRegla("+item.id+", \"" + item.descripcion + "\")" })%></td> 
