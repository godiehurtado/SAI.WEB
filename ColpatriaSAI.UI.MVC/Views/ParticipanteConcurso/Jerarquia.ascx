...
<style>
    ...
    <script type="text/javascript">
        ...
        function consultar(accion, inicioBusqueda) {
            ...
            $.ajax({ url: '/ParticipanteConcurso/getJerarquia', data: { texto: texto, inicio: inicio, cantidad: cantidad, nivel: nivel, zona: zona, concurso: concurso }, type: "POST",
                success: function (result) {
                    ...
                    $(result).each(function () {
                        $("<tr>" +
                            ...
                            "onclick='seleccionarJerarquia(" + this.id + ", \"" + this.nombre + " " + this.codigoNivel + " \",\"" + this.codigoNivel + "\");'  /></td>" +
                            "<td>" + this.nombre + "</td>" +                            
        ...
        function seleccionarJerarquia(id, nombre, codigoNivel) {
            $("#jerarquia").val(nombre);
    ...
    <%: Html.Label("Buscar")%>
    <input type="text" id="buscar" name="buscar" value="Digite Código de Nivel"
        onfocus="if (value == 'Digite Código de Nivel') { value = ''; }"
        onblur="if (value== '') { value = 'Digite Código de Nivel'; }" size="28" />
    ...
    <thead>
        <tr>
            <th>Selección</th><th>Nombres</th><th>Cod Nivel</th><th>Nivel</th>
        </tr>
    </thead>
    <tbody></tbody>
    </table>
